...
Mọi thao tác cập nhật dữ liệu đều diễn ra trong thư mục của từng tuyến đường, ví dụ: `routes/Mai Sơn - Quốc Lộ 45/`.
- `merged_Mai Sơn - Quốc Lộ 45.csv`: Chứa tọa độ các bốt thu phí.
...
## Case 1: Thêm một nhánh/bốt phụ vào Trạm đã có sẵn
...
```
**Quy tắc:**
- `"source"` và `"target"`: **Bắt buộc gõ chính xác** tên trạm (`epass_name`) đã khai báo ở file CSV.
...
### Bước 4: (Tùy chọn) Khai báo vào `nodes.json`
File này không bắt buộc đối với pipeline tìm đường của Python, nhưng thường dùng cho phía Frontend/Giao diện để hiển thị các điểm Node lớn.
```json
{
...
```
- **Bắt buộc:** Tọa độ (`latitude`, `longitude`), Tên trạm (`stationName`).
