...
EDGES_FILE = SCRIPT_DIR / "edges_expanded_restricted.json"
...
def main():
    ...
    print(
        f"Generated {OUTPUT_FILE.name} with {len(submission_list)} unique source OsmIds and {len(processed_pairs)} target connections.")

    # 5. Validate the completeness
    total_expanded_edges = len(edges_data)

    unique_epass_edges = set()
    for edge in edges_data:
        unique_epass_edges.add((edge["source"],edge["target"]))
    print(f"Unique epass edges: " f"{len(unique_epass_edges)}")
